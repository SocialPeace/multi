...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def set_ui(self):
        ...
        self.button_mode_1 = QtWidgets.QPushButton(u'포즈 추정 OFF')
        self.button_mode_2 = QtWidgets.QPushButton(u'multiperson recognition OFF')
        self.button_mode_3 = QtWidgets.QPushButton(u'action recognition OFF')

        self.button_close = QtWidgets.QPushButton(u'나가기')
        ...
        self.setWindowTitle(u'실시간 멀티 플레이어 포즈 추정 및 행동 인식 시스템')
    ...
    def button_event(self):
        ...
        if sender == self.button_mode_1 and self.timer_camera.isActive():
            if self.__flag_mode != 1:
                self.__flag_mode = 1
                self.button_mode_1.setText(u'포즈 추정 ON')
                self.button_mode_2.setText(u'multiperson recognition OFF')
                self.button_mode_3.setText(u'action recognition OFF')
            else:
                self.__flag_mode = 0
                self.button_mode_1.setText(u'포즈 추정 OFF')
                self.infoBox.setText(u'카메라 켜짐')
        elif sender == self.button_mode_2 and self.timer_camera.isActive():
            if self.__flag_mode != 2:
                self.__flag_mode = 2
                self.button_mode_1.setText(u'포즈 추정 OFF')
                self.button_mode_2.setText(u'multiperson recognition ON')
                self.button_mode_3.setText(u'action recognition OFF')
            else:
                self.__flag_mode = 0
                self.button_mode_2.setText(u'multiperson recognition OFF')
    ...
    def show_camera(self):
        ...
        if ret:
            if self.__flag_mode == 1:
                ...
                self.infoBox.setText(u'현재 인체 action recognition 모드')
                humans = poseEstimator.inference(show)
                ori = np.copy(show)
                show, joints, bboxes, xcenter, sk= TfPoseEstimator.get_skeleton(show, humans, imgcopy=False)
                height = show.shape[0]
    ...
    def closeEvent(self, event):
        ...
        msg = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, u"종료하시겠습니까?", u"종료 여부")
